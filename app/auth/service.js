  ...
  async loginOrSignUp(state, name, email, password) {
    ...
    try {
      if(state){
        ...
        const result = await this.apollo.mutate({ mutation: signInUserMutation, variables }, 'signInUser')
        this.saveUserData(result.user.id, result.token)
      } else {
        variables = { email, password }
        const result = await this.apollo.mutate({ mutation: createUser, variables }, 'signInUser').toPromise()
        this.saveUserData(result.user.id, result.token)
      }
    }
    catch(error) {
